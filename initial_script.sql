...
CREATE TABLE categories (
  ...
  slug VARCHAR UNIQUE NOT NULL,                   -- e.g., 'beer', used in code
  points INTEGER NOT NULL DEFAULT 1,              -- Point value per event
  icon VARCHAR,                                   -- e.g., 'Beer', from Lucide
  color VARCHAR,                                  -- Tailwind gradient class
  jersey_id UUID REFERENCES jerseys(id) ON DELETE CASCADE, -- Which jersey it affects
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);
